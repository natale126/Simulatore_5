...
st.markdown("Aggiungi o rimuovi gambe. Calcolo puro a scadenza futura senza l'errore matematico della volatilità residua.")
...
giorni_simulazione = st.sidebar.slider("Valuta la strategia tra X giorni:", min_value=0, max_value=30, value=9)
...
dati_iniziali = {
    ...
    "Scadenza (Giorni)": [9, 9, 10], 
    "Volatilità (%)": [16.0, 16.0, 16.0],
    "Prezzo d'Ingresso ($)": [1.50, 4.00, 2.35]
}
...
if len(df_gambe) > 0:
    ...
    fig.add_trace(go.Scatter(
        x=prezzi_simulati, y=payoff_totale, mode='lines', name=f'Payoff a {giorni_simulazione}gg',
    ...
    fig.update_layout(xaxis_title="Prezzo Sottostante", yaxis_title="Profitto / Perdita Totale ($)", template="plotly_dark", hovermode="x unified")
