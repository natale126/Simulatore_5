...
# --- 4. DOWNLOAD DATI ---
try:
    df_full = yf.download(ticker, start=data_ingresso, progress=False).ffill()
    if df_full.empty: raise ValueError("Nessun dato trovato")
    # Estrai il valore in modo sicuro
    prezzo_apertura = float(df_full['Close'].iloc[0].item() if isinstance(df_full['Close'].iloc[0], pd.Series) else df_full['Close'].iloc[0])
except Exception as e:
    st.error(f"Inserisci un Ticker valido (es. AAPL, TSLA). Errore: {e}")
    st.stop()
...
if giorni_totali <= 0: giorni_totali = 1

passaggio_tempo = st.sidebar.slider("Avanza nel tempo (Giorni)", 0, giorni_totali, 0)

idx_g = min(passaggio_tempo, len(df_full) - 1)
prezzo_corrente = float(df_full['Close'].iloc[idx_g].item() if isinstance(df_full['Close'].iloc[idx_g], pd.Series) else df_full['Close'].iloc[idx_g])
...
for i in range(2): # Simuliamo 2 gambe per semplicità
    with cols[i]:
        ...
        if scelta_scadenza and strikes:
            idx_c = min(range(len(strikes)), key=lambda j: abs(strikes[j]-prezzo_apertura))
            strike = st.selectbox(f"Strike", strikes, index=idx_c, key=f"s{i}")
        else:
            strike = st.number_input("Strike", value=float(round(prezzo_apertura, 2)), key=f"s{i}")
        
        # Logica Bid/Ask Reale
        iv, prezzo_entrata = 0.35, 0.0
        ...
        if chain is not None:
            ...
            if not row.empty:
                ...
                iv_reale = float(row['impliedVolatility'].iloc[0])
                iv = iv_reale if iv_reale > 0 else 0.35
                
                prezzo_entrata = ask if "Long" in pos else bid
                if prezzo_entrata == 0: # Fallback se bid/ask sono a zero
...
c2.metric("P&L Posizione", f"${pnl_totale:.2f}")
c3.metric("Data Corrente", data_corrente.strftime('%d %b %Y'))

# --- 9. GRAFICO PAYOFF ---
st.subheader("Profilo di Rischio a Scadenza")
...
fig = go.Figure()
fig.add_trace(go.Scatter(x=s_range, y=payoff_finale, name="Payoff a Scadenza", line=dict(color="#00ffcc", width=3)))
fig.add_vline(x=prezzo_corrente, line_dash="dash", line_color="orange", annotation_text="LIVE")
